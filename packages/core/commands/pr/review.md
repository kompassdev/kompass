...
- For deleted files, inspect the previous contents from git rather than assuming `changes_load` included the full file
...
- Call `pr_sync` with `refUrl: <pr-context.pr.url>` and `approve: true`
