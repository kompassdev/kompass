...
If `<has-supporting-feedback>` is false and `<previous-grade>` matches `<publish-grade>`:
- Skip publishing a review entirely
...
- If there are no inline comments and `<publish-grade>` changed, use only `<publish-grade>`
