...
- Turn `<planning-context>` and `<additional-context>` into `<plan-payload>`
- Ensure `<plan-payload>` includes:
  - a short title
  - a brief description
  - outcome-focused requirement items
  - outcome-focused validation items
- Keep the plan concise, human-friendly, and centered on scope, constraints, and expected outcomes

### Sync Ticket

- Use `kompass_ticket_sync` to store `<plan-payload>` in the ticket flow:
...
- Return the ticket reference, a one-sentence summary of the work, and the generated plan payload
