...
OpenCode normally runs commands as subtasks outside CI. Use `/commit/inline` or `/ship/inline` when the invoking session already has the required change context and should retain execution instead; these variants force `subtask: false` and skip their initial change load.
